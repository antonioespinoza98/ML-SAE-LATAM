...
encuesta_mrp <- readRDS("URY/2022/encuesta_mrp.rds")
statelevel_predictors_df <- readRDS("URY/2022/statelevel_predictors_df.rds")
...
censo <- readRDS("URY/2022/censo_mrp.rds") 
...
#- specify model
formula <- ingreso ~ luces_nocturnas + cubrimiento_rural + cubrimiento_urbano + modificacion_humana + accesibilidad_hospitales + accesibilidad_hosp_caminado + area1 + sexo2 + edad2 + edad3 + edad4 + edad5 + etnia2 + anoest2 + anoest3 + anoest3 + etnia1 + nbi_calefaccion + nbi_cocinar + nbi_educacion + nbi_habitable + nbi_materialida + nbi_saneamiento + nbi_vivienda + tiene_alcantarillado + tiene_sanitario+ tiene_electricidad + tiene_acueducto + tiene_agua_caliente + tiene_gas + tiene_internet + piso_tierra + material_paredes + material_techo + tasa_desocupacion
random <- ~ 1 | var_ale
...
sparse_matrix <- sparse.model.matrix(ingreso ~ .,
                                     data = dfsTrain[,-c(1,2,3,4,5,6,7,8,9,46)])[, -1]
...
saveRDS(xgb_train_rmse, file ="URY/output/xgb_train_rmse_all_tree.rds")
saveRDS(xgb_test_rmse, file = "URY/output/xgb_test_rmse_all_tree.rds")
...
# saveRDS(fitBoostMERT_L2, "URY/output/fit.rds")
# saveRDS(fhat_Test1, "URY/output/prediction.rds")
...
f <- readRDS("URY/output/prediction.rds")
length(f)
...
fit <- readRDS("URY/output/fit.rds")
...
# saveRDS(PBS, "URY/output/PBS.rds")
...
# saveRDS(PBS_long, "URY/output/PBS_long.rds")
# saveRDS(mean_df, "URY/output/mean_df.rds")
...
# saveRDS(final, "URY/output/bootstrap_results.rds")
