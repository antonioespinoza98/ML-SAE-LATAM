...
encuesta_mrp <- readRDS("PER/2022/encuesta_mrp.rds")
statelevel_predictors_df <- readRDS("PER/2022/statelevel_predictors_df.rds")
...
censo <- readRDS("PER/2022/censo_mrp.rds") 
...
#- specify model
formula <- ingreso ~ luces_nocturnas + cubrimiento_rural + cubrimiento_urbano + modificacion_humana + accesibilidad_hospitales + accesibilidad_hosp_caminado + area1 + sexo2 + edad2 + edad3 + edad4 + edad5 + etnia2 + discapacidad1 + anoest2 + anoest3 + anoest4 + etnia1 + tiene_alcantarillado + tiene_electricidad + tiene_acueducto + tiene_gas + tiene_internet + piso_tierra + material_paredes + material_techo + rezago_escolar + alfabeta + tasa_desocupacion
random <- ~ 1 | var_ale
...
sparse_matrix <- sparse.model.matrix(ingreso ~ .,
                                     data = dfsTrain[,-c(1,2,3,4,5,6,7,8,9,40)])[, -1]
...
saveRDS(xgb_train_rmse, file ="PER/output/xgb_train_rmse_all_tree.rds")
saveRDS(xgb_test_rmse, file = "PER/output/xgb_test_rmse_all_tree.rds")
...
# saveRDS(fitBoostMERT_L2, "PER/output/fit.rds")
# saveRDS(fhat_Test1, "PER/output/prediction.rds")
...
f <- readRDS("PER/output/prediction.rds")
length(f)
...
fit <- readRDS("PER/output/fit.rds")
...
# saveRDS(PBS, "PER/output/PBS.rds")
...
# saveRDS(PBS_long, "PER/output/PBS_long.rds")
# saveRDS(mean_df, "PER/output/mean_df.rds")

# Cálculo de medias para todas las PB -------------------------------------

PBS_long <- readRDS("PER/output/PBS_long.rds")
mean_df <- readRDS("PER/output/mean_df.rds")
...
# saveRDS(final, "PER/output/bootstrap_results.rds")

final <- readRDS("PER/output/bootstrap_results.rds")
...
ggsave(boot, filename = "PER/output/boot.jpg")
# Mapas -------------------------------------------------------------------
rm(list = ls())
