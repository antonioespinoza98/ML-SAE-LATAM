...
encuesta_mrp <- readRDS("PRY/2022/encuesta_mrp.rds")
statelevel_predictors_df <- readRDS("PRY/2022/statelevel_predictors_df.rds")
...
#- specify model
formula <- ingreso ~ luces_nocturnas + cubrimiento_rural + cubrimiento_urbano + modificacion_humana + accesibilidad_hospitales + accesibilidad_hosp_caminado + area1 + sexo2 + edad2 + edad3 + edad4 + edad5 + anoest2 + anoest3 + anoest4 + tiene_alcantarillado + tiene_sanitario + tiene_electricidad + tiene_acueducto + tiene_gas + eliminar_basura + tiene_internet + piso_tierra + material_paredes + material_techo + rezago_escolar + alfabeta + tasa_desocupacion
random <- ~ 1 | var_ale
...
saveRDS(xgb_train_rmse, file ="PRY/output/xgb_train_rmse_all_tree.rds")
saveRDS(xgb_test_rmse, file = "PRY/output/xgb_test_rmse_all_tree.rds")
