...
encuesta_mrp <- readRDS("GTM/2014/encuesta_mrp.rds")
statelevel_predictors_df <- readRDS("GTM/2014/statelevel_predictors_df.rds")
...
censo <- readRDS("GTM/2014/censo_mrp.rds") 
...
#- specify model
formula <- ingreso ~ luces_nocturnas + cubrimiento_rural + cubrimiento_urbano + modificacion_humana + accesibilidad_hospitales + accesibilidad_hosp_caminado + area1 + etnia2 + sexo2 + edad2 + edad3 + edad4 + edad5 + anoest2 + anoest3 + anoest4 + etnia1 + tiene_alcantarillado + tiene_electricidad + tiene_acueducto + tiene_gas + eliminar_basura + tiene_internet + piso_tierra + material_paredes + material_techo + rezago_escolar + alfabeta + hacinamiento + tasa_desocupacion
random <- ~ 1 | var_ale
...
saveRDS(xgb_train_rmse, file ="GTM/output/xgb_train_rmse_all_tree.rds")
saveRDS(xgb_test_rmse, file = "GTM/output/xgb_test_rmse_all_tree.rds")
...
# saveRDS(fitBoostMERT_L2, "GTM/output/fit.rds")
# saveRDS(fhat_Test1, "GTM/output/prediction.rds")
...
f <- readRDS("GTM/output/prediction.rds")
length(f)
...
fit <- readRDS("GTM/output/fit.rds")
...
# saveRDS(PBS, "GTM/output/PBS.rds")
...
# saveRDS(PBS_long, "GTM/output/PBS_long.rds")
# saveRDS(mean_df, "GTM/output/mean_df.rds")

# Cálculo de medias para todas las PB -------------------------------------

PBS_long <- readRDS("GTM/output/PBS_long.rds")
mean_df <- readRDS("GTM/output/mean_df.rds")
...
# saveRDS(final, "GTM/output/bootstrap_results.rds")

final <- readRDS("GTM/output/bootstrap_results.rds")
...
ggsave(boot, filename = "GTM/output/boot.jpg")
# Mapas -------------------------------------------------------------------
rm(list = ls())
