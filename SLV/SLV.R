...
encuesta_mrp <- readRDS("SLV/2022/encuesta_mrp.rds")
statelevel_predictors_df <- readRDS("SLV/2022/statelevel_predictors_df.rds")
...
censo <- readRDS("SLV/2022/censo_mrp.rds") 
...
#- specify model
formula <- ingreso ~ luces_nocturnas + cubrimiento_rural + cubrimiento_urbano + modificacion_humana + accesibilidad_hospitales + accesibilidad_hosp_caminado + area1 + sexo2 + edad2 + edad3 + edad4 + edad5 + etnia2 + anoest2 + anoest3 + anoest4 + etnia1 + tiene_alcantarillado + tiene_acueducto + tiene_gas + eliminar_basura + tiene_internet + piso_tierra + material_paredes + material_techo + rezago_escolar + alfabeta + tasa_desocupacion
random <- ~ 1 | var_ale
...
sparse_matrix <- sparse.model.matrix(ingreso ~ .,
                                     data = dfsTrain[,-c(1,2,3,4,5,6,7,8,38)])[, -1]
...
saveRDS(xgb_train_rmse, file ="SLV/output/xgb_train_rmse_all_tree.rds")
saveRDS(xgb_test_rmse, file = "SLV/output/xgb_test_rmse_all_tree.rds")
...
# saveRDS(fitBoostMERT_L2, "SLV/output/fit.rds")
# saveRDS(fhat_Test1, "SLV/output/prediction.rds")
...
f <- readRDS("SLV/output/prediction.rds")
length(f)
...
fit <- readRDS("SLV/output/fit.rds")
...
# saveRDS(PBS, "SLV/output/PBS.rds")
...
# saveRDS(PBS_long, "SLV/output/PBS_long.rds")
# saveRDS(mean_df, "SLV/output/mean_df.rds")

# Cálculo de medias para todas las PB -------------------------------------

PBS_long <- readRDS("SLV/output/PBS_long.rds")
mean_df <- readRDS("SLVoutput/mean_df.rds")
...
# saveRDS(final, "SLV/output/bootstrap_results.rds")

final <- readRDS("SLV/output/bootstrap_results.rds")
...
ggsave(boot, filename = "SLV/output/boot.jpg")
# Mapas -------------------------------------------------------------------
rm(list = ls())
