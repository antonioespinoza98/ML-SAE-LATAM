...
encuesta_mrp <- readRDS("PAN/2022/encuesta_mrp.rds")
statelevel_predictors_df <- readRDS("PAN/2022/statelevel_predictors_df.rds")
...
censo <- readRDS("PAN/2022/censo_mrp.rds") 
...
#- specify model
formula <- ingreso ~ luces_nocturnas + cubrimiento_rural + cubrimiento_urbano + modificacion_humana + accesibilidad_hospitales + accesibilidad_hosp_caminado + area1 + sexo2 + edad2 + edad3 + edad4 + edad5 + discapacidad1 + anoest2 + anoest3 + anoest4 + tiene_alcantarillado + tiene_sanitario + tiene_electricidad + tiene_acueducto + tiene_gas + eliminar_basura + tiene_internet + piso_tierra + material_paredes + material_techo + rezago_escolar + alfabeta + tasa_desocupacion
random <- ~ 1 | var_ale
...
sparse_matrix <- sparse.model.matrix(ingreso ~ .,
                                     data = dfsTrain[,-c(1,2,3,4,5,6,7,8,39)])[, -1]
...
saveRDS(xgb_train_rmse, file ="PAN/output/xgb_train_rmse_all_tree.rds")
saveRDS(xgb_test_rmse, file = "PAN/output/xgb_test_rmse_all_tree.rds")
...
saveRDS(fitBoostMERT_L2, "PAN/output/fit.rds")
# saveRDS(fhat_Test1, "PAN/output/prediction.rds")
...
f <- readRDS("PAN/output/prediction.rds")
length(f)
...
fit <- readRDS("PAN/output/fit.rds")
...
# saveRDS(PBS, "PAN/output/PBS.rds")
...
# saveRDS(PBS_long, "PAN/output/PBS_long.rds")
# saveRDS(mean_df, "PAN/output/mean_df.rds")

# Cálculo de medias para todas las PB -------------------------------------

PBS_long <- readRDS("PAN/output/PBS_long.rds")
mean_df <- readRDS("PAN/output/mean_df.rds")
...
# saveRDS(final, "PAN/output/bootstrap_results.rds")

final <- readRDS("PAN/output/bootstrap_results.rds")
...
ggsave(boot, filename = "PAN/output/boot.jpg")
# Mapas ---------------------------------upper# Mapas -------------------------------------------------------------------
rm(list = ls())
