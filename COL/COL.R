...
encuesta_mrp <- readRDS("COL/2021/encuesta_mrp.rds")
statelevel_predictors_df <- readRDS("COL/2021/statelevel_predictors_df.rds")
...
censo <- readRDS("COL/2021/censo_mrp.rds") 
...
#- specify model
formula <- ingreso ~ luces_nocturnas + cubrimiento_rural + cubrimiento_urbano + modificacion_humana + accesibilidad_hospitales + accesibilidad_hosp_caminado + area1 + sexo2 + edad2 + edad3 + edad4 + edad5 + etnia2 + anoest2 + anoest3 + anoest4 + etnia1 + tiene_alcantarillado + tiene_sanitario + tiene_electricidad + tiene_acueducto + tiene_gas + eliminar_basura + tiene_internet + piso_tierra + material_paredes + rezago_escolar + alfabeta + hacinamiento + tasa_desocupacion
random <- ~ 1 | var_ale
...
# saveRDS(fhat_Test1, "COL/output/prediction.rds")
...
f <- readRDS("COL/output/prediction.rds")
length(f)
...
fit <- readRDS("COL/output/fit.rds")
...
# saveRDS(PBS_long, "COL/output/PBS_long.rds")
# saveRDS(mean_df, "COL/output/mean_df.rds")

# Cálculo de medias para todas las PB -------------------------------------

PBS_long <- readRDS("COL/output/PBS_long.rds")
mean_df <- readRDS("COL/output/mean_df.rds")

medias <- numeric(33)
varianzas <- numeric(33)
...
# saveRDS(final, "COL/output/bootstrap_results.rds")

final <- readRDS("COL/output/bootstrap_results.rds")
